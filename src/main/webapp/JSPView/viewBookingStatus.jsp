  ...
  <div class="navbar">
    ...
    <div class="nav-links">
      ...
      <form action="/FlightReservationSystem/bookingController" method="post">
        ...
        <button type="submit" class="active">View Booking Status</button>
      </form>
...
<div class="container">
        ...
        <table>
            ...
            <%
                for (Booking booking : userBookings) {
                    ...
                    String statusClass = "status-failed";
                    if ("Pending".equals(booking.getStatus())) statusClass = "status-pending";
                    else if ("Completed".equals(booking.getStatus())) statusClass = "status-completed";
            %>
            <tr>
                <td><%= booking.getBookingCode() %></td>
                <td><%= booking.getFlight() != null ? booking.getFlight().getFlightNumber() : "N/A" %></td>
				<td><%= booking.getBookingDate() %></td>
                <td><%= booking.getNumOfPassengers() %></td>
                <td><%= booking.getTotalCost() %></td>
                <td class="<%= statusClass %>"><%= booking.getStatus() %></td>
               <%-- <td><%= payment != null ? payment.getStatus() : "Not Paid" %></td> --%>
            </tr>
            <%
                }
            %>
    ...
    %>
        <div class="error-message">No bookings found for your account.</div>
    <%
        }
    %>
